...
for kernel in kernels :
    ...
    train_accuracy = 1 - np.sum(np.abs(train_pred - Y_train)) / len(Y_train)
    valid_accuracy = 1 - np.sum(np.abs(valid_pred - Y_valid)) / len(Y_valid)
    test_accuracy = 1 - np.sum(np.abs(test_pred - Y_test)) / len(Y_test)
