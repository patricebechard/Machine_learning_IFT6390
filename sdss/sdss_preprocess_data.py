...
def retrieve_data():
    ...
    #command to download data : wget -i download_url.txt
...
def smooth_spectrum(data, n=15, a=1, npts=N_FEATURES):
...
def print_data_example():
    ...
    for line in f:
        ...
        np.savetxt('wd_spec_raw.txt',spec)
        np.savetxt('wd_spec_wavelength.txt',wv)
        ...
        np.savetxt('wd_spec_preprocessed.txt',spec)
...
def split_datasets():
    ...
    star_features = np.loadtxt('data/prepared_data.txt')

    np.random.shuffle(star_features)

    n_ex = star_features.shape[0]

    fmt = ['%.18e' for i in range(N_FEATURES)]
    fmt.append('%d')

    np.savetxt('data/sdss.train',star_features[:(3*n_ex//4)], delimiter=',',fmt=fmt)
    np.savetxt('data/sdss.valid',star_features[(3*n_ex//4):(7*n_ex//8)], delimiter=',',fmt=fmt)
    np.savetxt('data/sdss.test',star_features[(7*n_ex//8):], delimiter=',',fmt=fmt)
