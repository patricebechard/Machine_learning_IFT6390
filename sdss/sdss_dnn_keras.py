...
def train_model(training_set, valid_set, hidden_units,
    ...
    plt.savefig("figures/ecg_%s_%f.png"%(archstr, reg))
    plt.clf()

    np.savetxt("log/ecg_train_%s_%f.txt"%(archstr, reg), train_error)
    np.savetxt("log/ecg_test_%s_%f.txt"%(archstr, reg), test_error)
